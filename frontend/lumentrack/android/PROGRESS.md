...
*   **Seguridad y Sesión:** Integrado microservicio de autenticación (JWT) con persistencia en memoria y manejo de restricciones por rol.

## ✅ Implementado
- **Servicios Base:** `UsersService`, `MaterialService`, `OrdersService`, `SamplesService`, `ImagesService` (Cloudinary).
- **Autenticación:** Implementado `AuthService` (Puerto 8084) con soporte para JWT y LoginRequest/AuthResponse.
- **Gestión de Sesión:** Creado `SessionManager` (Singleton) para mantener el token y perfil de usuario "vivo" sin re-consultar la base de datos.
...
- **Control de Acceso:** Implementada `NoRoleScreen` para interceptar usuarios con `ROLE_NONE`, forzando el cierre de sesión y contacto con administración.

## 🛠️ En Proceso (El 10% Restante)
...
- [ ] **Seguridad Bearer:** Inyectar el token JWT en las cabeceras de los servicios existentes para habilitar la seguridad en el Backend.
- [ ] **Manejo de Errores Global:** Refinar interceptores para capturar el error 403 y redirigir al Login automáticamente.
